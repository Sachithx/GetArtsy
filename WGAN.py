...
# Define functions to Visualise Images
...
def generate_arts(n_arts=20):
    for art in range(n_arts):
        fake_images = the_model(get_noise(9, z_dim, device=device))
        fake_name = f'WGAN_Generated_{art}.png'
        save_image(denorm(fake_images), os.path.join(sample_dir, fake_name), nrow=3)
